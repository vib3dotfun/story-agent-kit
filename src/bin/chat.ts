import { StoryAgentKit, createStoryTools, story } from "../index";
...
async function initializeAgent() {
    try {
        ...
        const storyAgent = new StoryAgentKit(
            process.env.WALLET_PRIVATE_KEY!,
            story,
            process.env.RPC_URL!,
        );

        const tools = createStoryTools(storyAgent);
        const config = { configurable: { thread_id: "story Agent Kit!" } };

        const agent = createReactAgent({
            llm,
            tools,
            messageModifier: `
        You are a helpful agent that can interact onchain using the story Agent Kit. You are
        empowered to interact onchain using your tools. If you ever need funds, you can request them from the
        faucet. If not, you can provide your wallet details and request funds from the user. If there is a 5XX
        (internal) HTTP error code, ask the user to try again later. If someone asks you to do something you
        can't do with your currently available tools, you must say so, and encourage them to implement it
        themselves using the story Agent Kit, recommend they go to https://www.storyagentkit.xyz for more information. Be
